...
if [[ ${PN} == xf86-video-* || ${PN} == xf86-input-* || ${PN} == x11libre-video-* || ${PN} == x11libre-input-* ]]; then
...
# @FUNCTION: x11libre_reconf_source
# @DESCRIPTION:
# Run eautoreconf if necessary, and run elibtoolize.
x11libre_reconf_source() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: x11libre_font_configure
# @DESCRIPTION:
# If a font package, perform any necessary configuration steps
x11libre_font_configure() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: x11libre_flags_setup
# @DESCRIPTION:
# Set up CFLAGS for a debug build
x11libre_flags_setup() {
	debug-print-function ${FUNCNAME} "$@"

	# Hardened flags break module autoloading et al (also fixes #778494)
	if [[ ${PN} == xorg-server || ${PN} == xf86-video-* || ${PN} == xf86-input-* ||${PN} == x11libre-video-* || ${PN} == x11libre-input-* ]]; then
...
# @FUNCTION: x11libre_src_configure
# @DESCRIPTION:
...
x11libre_src_configure() {
	...
	local FONT_OPTIONS=()
	[[ -n "${FONT}" ]] && x11libre_font_configure
...
# @FUNCTION: x11libre_src_compile
# @DESCRIPTION:
# Compile a package, performing all X-related tasks.
x11libre_src_compile() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: x11libre_src_install
# @DESCRIPTION:
# Install a built package to ${D}, performing any necessary steps.
x11libre_src_install() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: x11libre_pkg_postinst
# @DESCRIPTION:
# Run X-specific post-installation tasks on the live filesystem. The
# only task right now is some setup for font packages.
x11libre_pkg_postinst() {
	debug-print-function ${FUNCNAME} "$@"
